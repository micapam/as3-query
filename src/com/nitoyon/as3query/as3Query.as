...
public class as3Query extends Proxy {
		...
		if(a is String) {
			...
			if(m && (m[1] || !c)) {
				...
				// HANDLE: $("#id")
					...
					var tmp:DisplayObject = Dom.getElementById( m[3] );
					if ( tmp ){
	...
	public function children( filter:String = null ):as3Query {
		return mapAndPush(filter, function(a:DisplayObject, ...args):Array {
			return as3Query.sibling(Dom.firstChild(a));
		});
	}
	...
	public function append(... args):as3Query {
		return domManip(args, true, 1, function(a:DisplayObject):void{
			this.addChild( a );
		});
	}
	...
	public function before(... args):as3Query {
		return this.domManip(args, false, 1, function(a:DisplayObject):void{
...
*/
	...
	public function domManip(args:Array, table:Boolean, dir:int, fn:Function):as3Query {
	...
	static public function nodeName( elem:DisplayObject, name:String ):Boolean {
		return Dom.nodeNameCmp( elem, name );
	}
...
/*	className: {
	...
	static public function attr( elem:Object, name:String, value:Object = null ):Object {
		...
		if( elem is DisplayObject && name == "id" )
			if( value != null && value is String ) {
				Dom.registerId( value.toString(), DisplayObject(elem) );
	...
	static private const expr:Object = {
		...
		":": {
			...
			// Child Checks
			"first-child": function(a:DisplayObject, ...args):Boolean{return Dom.firstChild(a.parent)==a},
			"last-child":  function(a:DisplayObject, ...args):Boolean{return Dom.lastChild(a.parent)==a},
			"only-child":  function(a:DisplayObject, ...args):Boolean{return a.parent.numChildren == 1},

			// Parent Checks
			parent: function(a:DisplayObject,i:int, m:Array, r:Array):Boolean{return !!Dom.firstChild(a)},
			empty:  function(a:DisplayObject,i:int, m:Array, r:Array):Boolean{return !Dom.firstChild(a)},
...
*/
			// Form elements
			text: function(a:DisplayObject,i:int, m:Array, r:Array):Boolean{return Dom.nodeNameCmp(a, "TextField")},
...
*/
		...
		while ( t && last != t ) {
			...
			if ( m ) {
				...
				for ( var i:int = 0; ret[i]; i++ )
					for ( var c:DisplayObject = Dom.firstChild(ret[i]); c; c = Dom.nextSibling(c) )
						if ( nodeName == "*" || Dom.nodeNameCmp(c, nodeName) )
							r.push( c );
				...
				if ( (m = re.exec(t)) != null ) {
					...
					for ( var j:int = 0, rl:int = ret.length; j < rl; j++ ) {
						var n:DisplayObject = m1 == "~" || m1 == "+" ? Dom.nextSibling(ret[j]) : Dom.firstChild(ret[j]);
						for ( ; n; n = Dom.nextSibling(n) ){
							if ( m1 == "~" && merge[n] ) break;

							if (!nodeName || Dom.nodeNameCmp(n, nodeName) ) {
								if ( m1 == "~" ) merge[n] = true;
								r.push( n );
							}
				...
				if ( !t.indexOf(",") ) {
					...
					if ( m[1] == "#" ) {//&& elem && elem.getElementById && !as3Query.isXMLDoc(elem) ) {
						...
						for ( i = 0; ret[i]; i++ ) {
							...
							r = as3Query.merge( r, Dom.getElementsByTagName( tag, ret[i] ));
						}
	...
	static public function dir( elem:DisplayObject, dir:String ):Array {
		var matched:Array = [];
		var cur:DisplayObject = Dom.getProperty(elem, dir) as DisplayObject;
		while ( cur && !(cur is Stage) ) {
			matched.push( cur );
			cur = Dom.getProperty(cur, dir) as DisplayObject;
		}
		return matched;
	}
	...
	static public function nth( cur:DisplayObject, result:Number, dir:String, elem:DisplayObject = null):DisplayObject {
		...
		for ( ; cur && !(cur is Stage); cur = Dom.getProperty(cur, dir) as DisplayObject)
			if ( ++num == result )
				break;
